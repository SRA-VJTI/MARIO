...
static const char GETPHYSICSPROPERTIES[] = "gazebo_msgs/GetPhysicsProperties";
  ...
  class GetPhysicsPropertiesResponse : public ros::Msg
  {
    ...
    virtual int deserialize(unsigned char *inbuffer)
    {
      ...
      u_max_update_rate.base |= ((uint32_t) (*(inbuffer + offset + 0))) << (8 * 0);
      u_max_update_rate.base |= ((uint32_t) (*(inbuffer + offset + 1))) << (8 * 1);
      u_max_update_rate.base |= ((uint32_t) (*(inbuffer + offset + 2))) << (8 * 2);
      u_max_update_rate.base |= ((uint32_t) (*(inbuffer + offset + 3))) << (8 * 3);
